...
class _GameScreenState extends State<GameScreen>
  ...
  void initState() {
    ...
    _tabController = TabController(
      length: 3,
      initialIndex: _isPreMatchStatus(currentSpielData['status']) ? 0 : 1,
      vsync: this,
    );
  }
  ...
  String _incidentTime(Map<String, dynamic> incident) {
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return ListTile(
            ...
            leading: Container(
              width: playerAvatarRadiusOnField * 2,
              height: playerAvatarRadiusOnField * 2,
              ...
              child: player.profileImageUrl == null ||
                  ...
                  ? Icon(Icons.person, size: playerAvatarRadiusOnField * 1.2)
                  : null,
            ),
            title: Row(
              children: [
                ...
                if (player.assists > 0)
                  const Padding(
                    padding: EdgeInsets.symmetric(horizontal: 2),
                    child: Icon(Icons.assistant, size: 16, color: Colors.blue),
                  ),
              ],
            ),
            trailing: Text(
  ...
  Widget _buildHeaderSummary() {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(
            '${DateFormat('dd.MM.yyyy').format(datum)} • ${DateFormat('HH:mm').format(datum)}',
  ...
  // Intentionally mirrors the original pitch implementation from spiel_screen.dart.
  Widget _buildPitchTab(BuildContext context) {
    ...
    final homeGkColor =
        _hexToColor(currentSpielData['home_goalkeeper_color_primary']) ??
            Colors.orange.shade700;
    final awayColor =
        _hexToColor(currentSpielData['away_color_primary']) ?? Colors.red.shade700;
    final awayGkColor =
        _hexToColor(currentSpielData['away_goalkeeper_color_primary']) ??
            Colors.orange.shade700;
    final homeFormation =
  ...
  Widget _buildEventsTab(BuildContext context) {
    ...
    return CustomScrollView(
      key: const PageStorageKey<String>('gameEventsTab'),
      physics: const AlwaysScrollableScrollPhysics(),
      slivers: [
        SliverOverlapInjector(
          handle: NestedScrollView.sliverOverlapAbsorberHandleFor(context),
