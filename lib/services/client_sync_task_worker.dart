...
class ClientSyncTaskWorker {
  ClientSyncTaskWorker({SupabaseClient? supabase, ApiService? apiService})
  ...
  void _ensureSession() {
    if (_cancelled ||
        _leaseError != null ||
        _supabase.auth.currentUser?.id != _taskUser) {
      throw StateError(
        'Sync wurde beendet oder die Reservierung ist abgelaufen.',
      );
    }
  }
  ...
  void start() {
    ...
    _isRunning = true;
    _loopFuture ??= _runLoop().whenComplete(() {
      _loopFuture = null;
      if (_isRunning) {
        _isRunning = false;
        start();
  ...
  Future<void> _runLoop() async {
    while (_isRunning) {
      ...
      try {
        ...
        debugPrint('Client-Sync-Worker: unerwarteter Fehler: $error');
  ...
  Future<bool> _processNextTask() async {
    ...
    try {
      ...
      debugPrint(
        'Client-Sync-Worker: Task konnte nicht beansprucht werden: $error',
      );
      return false;
    }
    ...
    final heartbeat = Timer.periodic(const Duration(minutes: 2), (_) async {
      try {
        _ensureSession();
        await _supabase.rpc('renew_sync_task', params: {'p_task_id': taskId});
      } catch (error) {
        _leaseError = error;
      }
    });
    try {
      ...
      debugPrint('Client-Sync-Worker: $taskType abgeschlossen.');
      return true;
    } catch (error, stackTrace) {
      debugPrint('Client-Sync-Worker: $taskType fehlgeschlagen: $error');
      debugPrint(stackTrace.toString());

      try {
        if (_supabase.auth.currentUser?.id != _taskUser) return false;
        await _supabase.rpc(
          'fail_sync_task',
        ...
        debugPrint(
          'Client-Sync-Worker: Fehlerstatus konnte nicht gespeichert werden: '
          '$reportError',
        );
      }
  ...
  Future<void> _executeTask(String taskType, Map<String, dynamic> task) async {
  ...
  Future<void> _fetchAndStoreTeams(int tournamentId, int seasonId) async {
    ...
    final teams = response['teams'] as List<dynamic>;
    ...
    for (final rawTeam in teams) {
      ...
      final teamData = <String, dynamic>{'id': teamId, 'name': teamName};
      if (imageUrl != null) teamData['image_url'] = imageUrl;

      _ensureSession();
      await _supabase.from('team').upsert(teamData, onConflict: 'id');
      _ensureSession();
      await _supabase.from('season_teams').upsert(<String, dynamic>{
        'season_id': seasonId,
        'team_id': teamId,
  ...
  Future<void> _processSquadPlayer(
    ...
    _ensureSession();
    await _supabase.from('spieler').upsert(<String, dynamic>{
      'id': playerId,
      'name': playerName,
    ...
    _ensureSession();
    await _supabase.from('season_players').upsert(<String, dynamic>{
      'season_id': seasonId,
      'player_id': playerId,
      'team_id': teamId,
      'is_active': true,
  ...
  Future<void> _fetchAndStoreAllMatches(int tournamentId, int seasonId) async {
  ...
  Future<void> _fetchAndStoreMatchesForRound(
    ...
    final events = validateMatchEvents(
    ...
    for (final rawEvent in events) {
      ...
      final status =
          type == 'finished'
              ...
              : 'nicht gestartet';
      final homeScore =
          event['homeScore'] is Map
              ? (event['homeScore'] as Map)['current']
              : null;
      final awayScore =
          event['awayScore'] is Map
      ...
      _ensureSession();
      await _supabase.from('spiel').upsert(<String, dynamic>{
        'id': matchId,
        'datum': date,
        ...
        'ergebnis': result,
        'status': status,
        'round': (event['roundInfo'] as Map)['round'],
        'season_id': (event['season'] as Map)['id'],
      }, onConflict: 'id');
    }
  }
  ...
  Future<void> _updateMatch(int matchId, int seasonId, int tournamentId) async {
    ...
    _ensureSession();
    final home = (event['homeScore'] as Map?)?['current'];
    final away = (event['awayScore'] as Map?)?['current'];
    await _supabase
        .from('spiel')
        .update({
          'status': status,
  ...
  Future<String?> _downloadAndStoreImage({
    ...
    try {
      ...
      debugPrint('Bild konnte nicht gespeichert werden ($path): $error');
      return null;
    }
  }
...
class _ApiLimitException implements Exception {
  ...
  @override
  String toString() => 'API_LIMIT_REACHED: SofaScore HTTP $statusCode für $url';
}
